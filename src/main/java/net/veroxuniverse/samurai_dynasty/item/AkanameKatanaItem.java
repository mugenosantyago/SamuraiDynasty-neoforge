...
public class AkanameKatanaItem extends ESWeaponItem {
    public AkanameKatanaItem(ToolMaterial pTier, float pAttackDamage, float pAttackSpeed, Properties pProperties) {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull Item.TooltipContext context, @NotNull TooltipDisplay tooltipDisplay, @NotNull Consumer<Component> components, @NotNull TooltipFlag flag) {
        if(Screen.hasShiftDown() && stack.getItem() == ItemsRegistry.KATANA_AKANAME.get()) {
            ...
            components.accept(Component.literal("§7Applies §bStrength§7.").withStyle(ChatFormatting.GRAY));
    ...
    public @NotNull InteractionResult use(@NotNull Level pLevel, @NotNull Player pPlayer, @NotNull InteractionHand pHand) {
        ...
        if (pHand == InteractionHand.MAIN_HAND) {
            pPlayer.getCooldowns().addCooldown(itemstack, 400);
            ...
            pPlayer.addEffect(new MobEffectInstance(MobEffects.STRENGTH, 200, 1, false, false, false), pPlayer);
        }
